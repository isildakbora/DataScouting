...
def th1_normalize_to_bins(h, name):

	h_new = h.Clone(name)
	h_new.Reset()

	for bin_idx in xrange(h.GetNbinsX()+1):

		x_bin_width = h.GetBinWidth(bin_idx)
		bin_content = h.GetBinContent(bin_idx)

		if x_bin_width != 0:
			new_content = float(bin_content)/float(x_bin_width)
			#print new_content
		else:
			new_content = 0
		h_new.SetBinContent(bin_idx, new_content)
