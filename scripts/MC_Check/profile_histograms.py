...
for eta in eta_bins[:-1]:
	...
	canvas = TCanvas("eta_" + str(int(eta)) + "_", "eta_" + str(int(eta)) + "_", 600, 600)
	can.append(canvas)
	gr_pT_vs_eta = []
	...
	for nPVz in nPVzbins[:-1]:
		...
		del h[:], x[:], ex[:], mean_new[:], RMS_new[:], err_on_mean[:]
	...
	can[-1].SaveAs( str(eta) + '_' + str(nPVz) + '_.pdf')
	can[-1].SaveAs( str(eta) + '_' + str(nPVz) + '_.C')
