...
parser.add_option("-s", "--signal",      action="store",      default="RSGravitonToGG_M_300", dest="signal")
...
file_to_write = TFile(signal + '_HLT_RECO_Comp_nPVz_test.root', 'RECREATE')
#if working local
...
for k in range(len(eta_bins)-1):
	for j in range(len(nPVzbins)-1):
		for i in range(len(res_bins)-1):
			common_suffixes    = signal + "_" + str(nPVzbins[j]) + '_' + str(nPVzbins[j+1]) + '_' + str(int(res_bins[i])) + "_" + str(int(res_bins[i+1])) + "_" + str(eta_bins[k]) + "_" + str(eta_bins[k+1])
...
for jentry in range(0, int(entries/2)):
