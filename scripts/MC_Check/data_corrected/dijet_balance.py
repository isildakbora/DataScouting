...
def RECO_2_HLT_Corr(recoJetEta, recoJetPt, signal):
	...
	if recoJetEta < eta_binning[-1] and recoJetPt < res_bins[-1]:
		...
		root_file  = TFile('ratio_data.root')
...
h_ds_Mjj_raw = TH1F("h_ds_Mjj_raw", "h_ds_Mjj_raw", len(massBins)-1 , massBins)
...
for jentry in range(int(entries/2), entries):
	...
	DSjet1_raw = TLorentzVector(0.,0.,0.,0.)
	DSjet2_raw = TLorentzVector(0.,0.,0.,0.)
	auxJet     = TLorentzVector(0.,0.,0.,0.)

	DSjet1_raw.SetPtEtaPhiE(dsJetPt[0],dsJetEta[0],dsJetPhi[0],dsJetE[0])
	DSjet2_raw.SetPtEtaPhiE(dsJetPt[1],dsJetEta[1],dsJetPhi[1],dsJetE[1])
	...
	if useWideJets:
		...
		for i in xrange(2, nDSJets):
			...
			deltaR1 = auxJet.DeltaR(DSjet1_raw)
			deltaR2 = auxJet.DeltaR(DSjet2_raw)
		...
		if DSjet2_raw.Pt() > DSjet1_raw.Pt():
			auxJet  = DSjet2
			DSjet2_raw = DSjet1_raw
			DSjet1_raw = auxJet

	dsJetPt[0]      = RECO_2_HLT_Corr(dsJetEta[0], dsJetPt[0], signal)
	dsJetPt[1]      = RECO_2_HLT_Corr(dsJetEta[1], dsJetPt[1], signal)
	...
	if useWideJets:
		...
		#########################################################################
		for i in xrange(2, nDSJets):
			auxJet.SetPtEtaPhiE(RECO_2_HLT_Corr(dsJetEta[i],dsJetPt[i]),dsJetEta[i],dsJetPhi[i],dsJetE[i])
	...
	ds_dijet_mass_raw = (DSjet1_raw + DSjet2_raw).M()
	h_reco_Mjj.Fill(reco_dijet_mass)
	h_ds_Mjj.Fill(ds_dijet_mass)
	h_ds_Mjj_raw.Fill(ds_dijet_mass_raw)
    #// Calculate dijet mass //
    #########################################################################
...
h_reco_Mjj.Draw()
h_ds_Mjj.SetLineColor(ROOT.kBlue)
h_ds_Mjj.SetLineWidth(2)
h_ds_Mjj.Draw("SAME")	
h_ds_Mjj_raw.SetLineColor(ROOT.kBlack)
h_ds_Mjj_raw.SetLineWidth(2)
h_ds_Mjj_raw.Draw("SAME")
...
ratio_reco_ds.SetLineColor(ROOT.kBlue)
ratio_reco_ds_raw = h_reco_Mjj.Clone("ratio_reco_ds_raw")

ratio_reco_ds.Divide(h_ds_Mjj)
ratio_reco_ds_raw.Divide(h_ds_Mjj_raw)
ratio_reco_ds.SetTitle("ratio_reco_ds")
ratio_reco_ds_raw.SetTitle("ratio_reco_ds_raw")

ratio_reco_ds.GetYaxis().SetTitle("Ratio")
ratio_reco_ds.Draw()
ratio_reco_ds_raw.Draw("SAME")
legend = TLegend(0.6,0.4,0.9,0.6)
legend.SetFillColor(ROOT.kWhite)
legend.AddEntry(ratio_reco_ds)
legend.AddEntry(ratio_reco_ds_raw)
legend.Draw("SAME")
