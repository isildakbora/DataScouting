...
for eta in eta_bins[:-1]:
	...
	can1[-1].SaveAs( "eta" + str(eta) + '.pdf')

	can2[-1].cd()
	gr_pT_vs_eta_rms_per_mean[0].SetTitle('rms_per_mean_pT_vs_ratio_in_eta_' + str(eta) + '_' + str(eta_bins[bisect_left(eta_bins, eta)+1]))
	...
	can2[-1].SaveAs( "rms_per_mean_eta_" + str(eta) + '.pdf')
