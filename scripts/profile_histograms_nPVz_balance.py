...
for eta in eta_bins[:-1]:
	canvas1 = TCanvas(str(eta), str(eta), 11, 51, 600, 600)
	can1.append(canvas1)
	canvas2 = TCanvas(str(eta)+"_", str(eta)+"_", 11, 51, 600, 300)
	can2.append(canvas2)
	gr_pT_vs_eta = []
	...
	for nPVz in nPVzbins[:-1]:
		...
		for obj in dirList: 
			...
			is_HLT	      = "HLT"  in histo.GetName().split('_')
			...
			if  is_balance and (not is_raw) and (float(eta_bin_low) == eta) and (int(nPVz) == int(nPVz_bin_low)):
				index = bisect_left(res_bins, float(pT_bin_low))
				h[index].Add(histo)
				h[index].SetName(histo.GetName())

			if  is_balance and (not is_raw) and (float(eta_bin_low) == eta) and (int(nPVz) < 25):
	...
	can1[-1].cd()
	gr_pT_vs_eta[0].SetTitle('dijet_balance_' + str(eta) + '_' + str(eta_bins[bisect_left(eta_bins, eta)+1]))
	...
	can2[-1].cd()
	gr_pT_vs_eta_rms_per_mean[0].SetTitle('dijet_balance_rms_per_mean' + str(eta) + '_' + str(eta_bins[bisect_left(eta_bins, eta)+1]))
	...
	can2[-1].SaveAs( "dijet_balance_rms_per_mean" + str(eta) + '.pdf')
can = []
for h in h_balance:
